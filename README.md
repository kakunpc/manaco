...
アバタープリセットをasset形式で保存・配布することができます。  
assetファイルであるため、本スクリプトを導入していないユーザーがアバターをアップロードしてもスクリプトエラーが発生しないつくりとなっています。
...
Blenderなどで書き出せるUVMap画像データの目の部分だけが数値としてassetファイルに保存されているようなイメージとなります。  

### Eyeテクスチャ・シェーダー作者の方

テクスチャのマテリアル割当をasset形式で保存・配布することができます。  
assetファイルであるため、本スクリプトを導入していないユーザーがアバターをアップロードしてもスクリプトエラーが発生しないつくりとなっています。
